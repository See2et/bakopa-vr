...
## Introduction
本仕様は、Godot Engine と bevy_ecs を組み合わせ、GDExtension (gdext) を通じて Godot から ECS を呼び出せる最小クライアントを対象とする。SteamVR で起動できることを到達点とし、状態の真実は bevy_ecs に集中させ、Godot は描画中心に保つ。

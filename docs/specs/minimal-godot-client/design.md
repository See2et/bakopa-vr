...
##### Responsibilities & Constraints (GodotBridgeAdapter)

- ゲーム状態の正本と更新順序の維持
- Godot 依存を持たない純 Rust の境界
- 依存性逆転のための抽象インターフェースのみ公開

##### Dependencies (GodotBridgeAdapter)
...
##### Service Interface (GodotBridgeAdapter)
...
##### Responsibilities & Constraints (RenderStateProjector)

- XR ランタイムの有効化と初期化結果の通知
- SteamVR 未起動時の明示的な失敗理由提示

##### Dependencies (RenderStateProjector)
...
##### Responsibilities & Constraints (ClientBootstrap)

- RenderFrame を Godot ノードへ投影
- Godot 側は描画専用でゲーム状態は保持しない

##### Dependencies (ClientBootstrap)
...
##### Service Interface (ClientBootstrap)
